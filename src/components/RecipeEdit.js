...
export default function RecipeEdit() {
  ...
  async function editRecipe(event) {
    ...
    const recipeDocRef = doc(db, 'recipes', recipe.data.id)
    try {
      await updateDoc(recipeDocRef, {
        id: recipe.data.id,
        name: event.target.elements.name.value,
        slug: convertToSlug(event.target.elements.name.value),
        ingredients: ingredients,
        steps: steps,
        imageURL: event.target.elements.imgURL.value
      })
      navigate("/receptek")
    } catch (err) {
      alert(err)
        ...
        <>
          ...
          <form data-testid="recipe-form-submit" onSubmit={editRecipe}
            ...
            <div key="input-row-img-url" className="input-row">
              <label>Kép:</label>
              <input
                data-testid="recipe-img-url"
                style={{ marginTop: "10px" }} className={isTabletOrMobile
                  ? "input-img-mobile" : "input-img"} type="file"
                id="img-url" name="img-url" />
              <img className={isTabletOrMobile
                ? "input-row-img-src-mobile" : "input-row-img-src"}
                src={recipe.data.imageURL} alt={recipe.data.name}
              />
            </div>
